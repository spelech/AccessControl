...
public class DoorOperationService : IDoorOperationService
{
    ...
    private ILockProvider CreateLockProvider(AccessPoint door)
    {
        ...
        if (lockType.Contains("ZWave", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                var wsTransport = _transportRegistry.GetTransport<AccessControl.Core.Transports.ILockTransport>("zwave_ws");
                if (wsTransport != null && wsTransport.IsConnected && !string.IsNullOrEmpty(target) && AccessControl.Engine.Transports.ZWaveWebSocketTransport.TryParseNodeId(target, out var nodeId))
                {
                    return new AccessControl.Engine.Transports.TransportLockProviderAdapter(wsTransport, nodeId.ToString());
                }

                var mqttTransport = _transportRegistry.GetTransport<AccessControl.Core.Transports.ILockTransport>("zwave_mqtt");
                if (mqttTransport != null && mqttTransport.IsConnected)
                {
                    return new AccessControl.Engine.Transports.TransportLockProviderAdapter(mqttTransport, target ?? door.Id);
                }
            }
