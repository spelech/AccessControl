...
namespace CodeMaster.Mcp;

public interface ICodeMasterMcpServer
{
...
public class CodeMasterMcpServer : ICodeMasterMcpServer
{
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<CodeMasterMcpServer> _logger;
    private readonly ConcurrentDictionary<string, ChannelWriter<string>> _activeSessions = new();

    public CodeMasterMcpServer(IServiceScopeFactory scopeFactory, ILogger<CodeMasterMcpServer> logger)
    {
...
{
    public static IServiceCollection AddCodeMasterMcp(this IServiceCollection services)
    {
        services.AddScoped<IDoorTools, DoorTools>();
        services.AddSingleton<ICodeMasterMcpServer, CodeMasterMcpServer>();
        return services;
    }

    public static IEndpointRouteBuilder MapMcpEndpoints(this IEndpointRouteBuilder endpoints)
    {
        endpoints.MapGet("/mcp/sse", async (HttpContext context, ICodeMasterMcpServer server, CancellationToken ct) =>
        {
            await server.HandleSseConnectionAsync(context, ct);
        });

        endpoints.MapPost("/mcp/messages", async (HttpContext context, ICodeMasterMcpServer server, CancellationToken ct) =>
        {
        ...
        endpoints.MapGet("/sse", async (HttpContext context, ICodeMasterMcpServer server, CancellationToken ct) =>
        {
            await server.HandleSseConnectionAsync(context, ct);
        });
