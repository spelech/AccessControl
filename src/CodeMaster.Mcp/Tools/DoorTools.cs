...
namespace CodeMaster.Mcp.Tools;
...
public class DoorTools : IDoorTools
{
        ...
        [
            ...
            new McpToolDefinition
            {
                ...
                InputSchema = new
                {
                    ...
                    properties = new
                    {
                        name = new { type = "string", description = "Name of the guest" },
                        pin = new { type = "string", description = "4-8 digit numeric PIN" },
                        validFrom = new { type = "string", description = "Start time in ISO 8601 UTC format (e.g. 2026-09-06T10:00:00Z)" },
                        validUntil = new { type = "string", description = "End time in ISO 8601 UTC format (e.g. 2026-09-07T18:00:00Z)" },
                        doorIds = new
                        {
                            type = "array",
                            items = new { type = "string" },
                            description = "List of door IDs accessible with this PIN"
                        }
                    },
                    required = new[] { "name", "pin", "validFrom", "validUntil", "doorIds" }
                }
            },
            new McpToolDefinition
            {
                Name = "codemaster__revoke_user",
                Description = "Deactivates a user and immediately clears hardware slots across all locks.",
                InputSchema = new
                {
                    type = "object",
                    properties = new
    ...
    public async Task<McpToolCallResult> ExecuteToolAsync(string toolName, JsonElement? arguments, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                "codemaster__list_doors" => await ListDoorsAsync(ct),
                "codemaster__unlock_door" => await UnlockDoorAsync(arguments, ct),
                "codemaster__lock_door" => await LockDoorAsync(arguments, ct),
                "codemaster__create_guest_pin" => await CreateGuestPinAsync(arguments, ct),
                "codemaster__revoke_user" => await RevokeUserAsync(arguments, ct),
                "codemaster__get_access_logs" => await GetAccessLogsAsync(arguments, ct),
