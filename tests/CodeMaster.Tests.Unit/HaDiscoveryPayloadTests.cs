...
public class HaDiscoveryPayloadTests
{
    ...
    public void BuildEventDiscoveryPayload_ReturnsValidJson_WithExpectedSchema()
    {
        ...
        Assert.Equal("homeassistant/event/codemaster_side_door/config", topic);
        Assert.Contains("homeassistant/event/codemaster_side_door/config", payload);

        using var doc = JsonDocument.Parse(payload);
        var root = doc.RootElement;

        Assert.Equal("Side Door Access", root.GetProperty("name").GetString());
        Assert.Equal("codemaster_side_door_access", root.GetProperty("unique_id").GetString());
        Assert.Equal("codemaster/side_door/event/state", root.GetProperty("state_topic").GetString());
        ...
        Assert.Contains("codemaster_side_door", identifiers);
        Assert.Equal("CodeMaster", device.GetProperty("manufacturer").GetString());
    ...
    public void BuildLockDiscoveryPayload_ReturnsValidJson_WithExpectedSchema()
    {
        ...
        Assert.Equal("homeassistant/lock/codemaster_side_door/config", topic);

        using var doc = JsonDocument.Parse(payload);
        var root = doc.RootElement;

        Assert.Equal("Side Door", root.GetProperty("name").GetString());
        Assert.Equal("codemaster_side_door_lock", root.GetProperty("unique_id").GetString());
        Assert.Equal("codemaster/side_door/lock/state", root.GetProperty("state_topic").GetString());
        Assert.Equal("codemaster/side_door/lock/set", root.GetProperty("command_topic").GetString());
    ...
    public void BuildSensorDiscoveryPayload_ReturnsValidJson_WithDoorDeviceClass()
    {
        ...
        Assert.Equal("homeassistant/binary_sensor/codemaster_side_door/config", topic);

        using var doc = JsonDocument.Parse(payload);
        var root = doc.RootElement;

        Assert.Equal("Side Door Sensor", root.GetProperty("name").GetString());
        Assert.Equal("codemaster_side_door_sensor", root.GetProperty("unique_id").GetString());
        Assert.Equal("codemaster/side_door/sensor/state", root.GetProperty("state_topic").GetString());
    ...
    public void BuildAutoLockSwitchDiscoveryPayload_ReturnsValidJson_WithSwitchSchema()
    {
        ...
        Assert.Equal("homeassistant/switch/codemaster_side_door/config", topic);

        using var doc = JsonDocument.Parse(payload);
        var root = doc.RootElement;

        Assert.Equal("Side Door Auto-Lock", root.GetProperty("name").GetString());
        Assert.Equal("codemaster_side_door_autolock", root.GetProperty("unique_id").GetString());
        Assert.Equal("codemaster/side_door/autolock/state", root.GetProperty("state_topic").GetString());
        Assert.Equal("codemaster/side_door/autolock/set", root.GetProperty("command_topic").GetString());
    }
